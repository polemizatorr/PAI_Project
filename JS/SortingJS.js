    ...
    function denyAdmin(Username) {
        if (!confirm('Do you want to deny Admin permission to this Admin?')) {
            return;
        }
        const apiUrl = "http://localhost:8080";
        $.ajax({
            url : apiUrl + '/?page=admin_deny_admin',
            method : 'POST',
            dataType: 'json',
            data : {
                Username : Username
    ...
    function getOnlyAdmins() {
        ...
        const $list = $('.users-list');
        $.ajax({
            url : apiUrl + '/?page=admin_admins_only',
            dataType : 'json',
            ...
            .done((res) => {
                ...
                res.forEach(el => {
                    $list.append(`<tr>
                         ...
                         <td>${el.Role}</td>
                         <td><button class="btn btn-danger" type="button" onclick="deleteUser('${el.Username}')"><i class="material-icons">Delete User</i></button>
                         <button class="btn btn-success" type="button" onclick="denyAdmin('${el.Username}')"><i class="material-icons">Deny Admin</i></button></td>
